...
if [[ "$DEV" -gt 0 ]]; then
    ...
    DIR_COLORSCRIPTS="`echo $HOME`/scripts/colorscripts/colorscripts"
fi
...
function _random() {
    ...
    . ${DIR_COLORSCRIPTS}/${random_colorscript}
}
...
function _blacklist_colorscript() { # by name only
    if [ ! -d "${DIR_COLORSCRIPTS}/blacklisted" ]; then
         mkdir "${DIR_COLORSCRIPTS}/blacklisted"
    fi
     mv "${DIR_COLORSCRIPTS}/$1" "${DIR_COLORSCRIPTS}/blacklisted"
}
